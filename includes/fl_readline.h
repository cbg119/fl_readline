...
/*   Updated: 2019/05/10 16:30:15 by cbagdon          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
# define CMD_MAX 4096
# define LINE_TABLE_ENTRIES 10
# define LINE_C_TABLE_ENTRIES 6
...
typedef void					(*fl_line)(t_line *line);
typedef void					(*fl_line_c)(t_line *line, unsigned long c);

typedef struct					s_line_dispatch
{
	unsigned long				key;
	const fl_line				move;
}								t_line_dispatch;

typedef struct					s_c_dispatch
{
	unsigned long				key;
	const fl_line_c				move;
}								t_c_dispatch;

int								fl_match_line_key(unsigned long key, t_line *line);
int								fl_match_line_c_key(unsigned long key, t_line *line);
int								fl_match_history_key(unsigned long key,
								t_line *line, t_h_list *history);
...
t_line							g_line;
extern const t_line_dispatch	g_line_table[];
extern const t_c_dispatch		g_line_c_table[];
